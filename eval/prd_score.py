...
# Based on: https://github.com/raahii/video-gans-evaluation/blob/master/metrics/precision_recall_distributions.py
...
def compute_embeddings(data_generator, model):
    features = []
...
def prd_score(data1, data2, dims=1000, use_cuda=True):
    """Calculates the PRD of two data generator.
    Params:
    ...
    f1 = compute_embeddings(data1, model)
    f2 = compute_embeddings(data2, model)

    return compute_prd_from_embedding(f1, f2)
