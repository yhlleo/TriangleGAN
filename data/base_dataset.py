...
class BaseDataset(data.Dataset, ABC):
    ...
    def _get_params(self, angle=15, scale=0.3, shift=32, shear=8):
        assert angle>=0, angle
        assert scale>=0, scale
        assert shift>=0, shift
        assert shear>=0, shear

        out_angle = random.uniform(-angle, angle)
    ...
    def _im_trans(self, im, params):
        return TF.affine(Image.fromarray(im), *params)

    def _id2arr(self, idx, dims):
